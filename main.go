...
func CMP(addressingMode string) {
	...
	case ZEROPAGEX: // Zero Page, X
		...
		// Set N flag to true if A minus operand results in SR negative bit 7 being set, else reset N flag
		if readBit(7, A-value) == 1 {
			setNegativeFlag()
		} else {
			unsetNegativeFlag()
		}
...
func ROR(addressingMode string) {
	...
	case ABSOLUTE:
		...
		SR = (SR & 0xFE) | byte(readBit(0, value))
...
func ROL(addressingMode string) {
	...
	// Set SR negative flag to bit 6 of value
	if readBit(6, value) == 1 {
		setNegativeFlag()
	} else {
		unsetNegativeFlag()
