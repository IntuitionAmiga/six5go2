...
func main() {
	...
	if len(os.Args) > 2 && os.Args[2] == "hex" {
		printHex = true
	}
	...
	fmt.Printf("All 256 documented and illegal opcodes are fully implemented\n\n")
	fmt.Printf("USAGE : six5go2 <target_filename> hex (Optional parameter to also print hex values on line above each instruction disassembly)\n\n")
...
func disassemble(file string) {
	for counter := 0; counter <= len(file); {
		...
		case 0x82:
			if printHex {
				fmt.Printf(";; $%04x\t$%02x $%02x\t\t(Stack Rela Indirect,Y)\n", counter, file[counter], file[counter+1])
			}
		...
		case 0x0C:
			if printHex {
				fmt.Printf(";; $%04x\t$%02x $%02x $%02x\t(Absolute)\t\n", counter, file[counter], file[counter+1], file[counter+2])
			}
		...
		case 0x13:
			if printHex {
				fmt.Printf(";; $%04x\t$%02x $%02x $%02x\t(Rela (word))\t\n", counter, file[counter], file[counter+1], file[counter+2])
			}
