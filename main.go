...
func execute(file string) {
	...
	for fileposition = 0; fileposition < len(file); {
		...
		case 0x50:
			...
			fmt.Printf("BVC $%02X\n", fileposition+2+int(memory[fileposition+1]))
			incCount(2)
		case 0x51:
			if printHex {
				fmt.Printf(";; $%04x\t$%02x $%02x\t\t((Zero Page Indirect),Y)\n", PC, memory[fileposition], memory[fileposition+1])
		...
		case 0x85:
			...
			*/
			...
			memory[memory[fileposition+1]] = A
			//fmt.Printf("Address[$%02X] = $%02X\n", memory[fileposition+1], memory[memory[fileposition+1]])
			incCount(2)
			printMachineState()
		case 0x86:
			if printHex {
		...
		case 0xC9:
			...
			*/
			...
			if (A-memory[fileposition+1])&0b10000000 == 0b10000000 {
				//Set bit 7 to true
				SR |= 1 << 7
				//SR = 0b10100000
			}
			//Set N flag to false if A minus operand results in most significant bit being unset
			if (A-memory[fileposition+1])&0b10000000 == 0b00000000 {
				//Set bit 7 to false
				SR |= 0 << 7
				//SR = 0b00100000
			}
		...
		case 0x8D:
			...
			fmt.Printf("STA $%04X\n", memory[fileposition+1]|uint8(int(memory[fileposition+2])<<8))
			incCount(3)
		case 0x8E:
			if printHex {
				fmt.Printf(";; $%04x\t$%02x $%02x $%02x\t(Absolute)\t\n", PC, memory[fileposition], memory[fileposition+1], memory[fileposition+2])
		...
		case 0xBC:
			...
			*/
			...
			X = memory[fileposition+2]
			Y = memory[int(memory[fileposition+1])+int(X)]
