...
func getMnemonic(opcode byte) string {
	...
	// Zero Page
	case 0x65, 0x25, 0x05, 0x24, 0xA5, 0xA6, 0xA4, 0x06, 0x46, 0xE6, 0xC6, 0x85, 0x86, 0x84, 0xC5, 0x45, 0x26, 0x66, 0xE5, 0xD5, 0xE4, 0xC4:
		return fmt.Sprintf("%s $%02X", mnemonic, cpu.preOpOperand1)
	// Zero Page,X
	case 0x75, 0x35, 0x15, 0xB5, 0xB4, 0x16, 0x56, 0xF6, 0xD6, 0x95, 0x94, 0x81, 0x55, 0x36, 0x76, 0xF5, 0xC1:
		return fmt.Sprintf("%s $%02X,X", mnemonic, cpu.preOpOperand1)
	// Zero Page,Y
	case 0xB6, 0x96, 0xD1:
		return fmt.Sprintf("%s $%02X,Y", mnemonic, cpu.preOpOperand1)
	// Absolute
	case 0x6D, 0x2D, 0x0D, 0x2C, 0xAD, 0xAE, 0xAC, 0x0E, 0x4E, 0xEE, 0xCE, 0x8D, 0x8E, 0x8C, 0x4C, 0x20, 0xCD, 0x4D, 0x2E, 0x6E, 0xED, 0xEC, 0xCC:
		address = uint16(cpu.preOpOperand2)<<8 | uint16(cpu.preOpOperand1)
		return fmt.Sprintf("%s $%04X", mnemonic, address)
	// Absolute,X
	case 0x7D, 0x3D, 0x1D, 0xBD, 0xBC, 0x1E, 0x5E, 0xFE, 0xDE, 0x9D, 0xDD, 0x5D, 0x3E, 0x7E, 0xFD:
		address = uint16(cpu.preOpOperand2)<<8 | uint16(cpu.preOpOperand1)
		return fmt.Sprintf("%s $%04X,X", mnemonic, address)
	// Absolute,Y
	case 0x79, 0x39, 0x19, 0xB9, 0xBE, 0x99, 0x91, 0xD9, 0x59, 0xF9:
		address = uint16(cpu.preOpOperand2)<<8 | uint16(cpu.preOpOperand1)
	...
	case 0x0A, 0x4A, 0x2A, 0x6A, 0xEA, 0x40, 0x60, 0x18, 0xD8, 0x58, 0xB8, 0xCA, 0x88, 0xE8, 0xC8, 0x48, 0x08, 0x68, 0x28, 0x78, 0xAA, 0xA8, 0xBA, 0x8A, 0x9A, 0x98, 0x38, 0xF8:
